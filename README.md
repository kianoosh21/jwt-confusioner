...
python3 jwt-confusioner.py -t '<YOUR_JWT>' --jwks-url 'https://idp.example.com/.well-known/jwks.json'

3) Provide a single public key (PEM or DER) directly  
python3 jwt-confusioner.py -t '<YOUR_JWT>' --public-key-url 'https://idp.example.com/public.pem'
...
python3 jwt-confusioner.py -t '<YOUR_JWT>' --jwks-url 'https://idp.example.com/jwks.json' --pretty-blank-lines -o tokens.json
